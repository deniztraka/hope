...
public class DragAndDropCell : MonoBehaviour, IDropHandler
{
        ...
        if (item != null)
        {
            ...
            if (cell != null)
            {
                if (cell.unlimitedSource == true)
                {
                    ...
                    item.transform.SetParent(transform.Find("SlotWrapperCanvas").Find("SlotWrapperPanel"), false);                              // Clone item from source cell
                    item.name = itemName;
                }
            }
    ...
    {
        var slotBehaviour = GetComponent<SlotBehaviour>();
